...
export default function ImageInput({ setConvertedUrl }) {
  ...
  const handleConvert = async () => {
    ...
    if (
      lastConversion.signature === generateSignature(selectedFile) &&
    ...
    >
      ...
      {previewUrl ? (
        ...
        <>
          <div
            {...getRootProps()}
            className={`w-full p-6 border-2 border-dashed rounded-lg bg-white hover:bg-gray-100 cursor-pointer text-center transition-colors duration-300 ${
              selectedFile ? "border-green-400" : "border-indigo-400"
            }`}
          >
